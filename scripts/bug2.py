...
def callback(data):
      ...
      for i in range(361):
              ...
              if data.intensities[i] == 1.0:
                   s_wall = True
      if data.intensities[0] == 1.0 and data.ranges[0] <= 0.8:
              s_r_wall = True
      else:
              s_r_wall = False
      if data.intensities[360] == 1.0 and data.ranges[360] <= 0.8:
              s_l_wall = True
      else:
              s_l_wall = False
...
def robot0():
      ...
      while not rospy.is_shutdown():
	    ...
	    line_pub.publish(line_marker)
            ...
            #print("s_l_wall: ", s_l_wall, "  s_r_wall: ", s_r_wall)
            #print(move_tw)
            ...
            if round(robot_position[0], 1) == 4.5 and round(robot_position[1], 1) == 9.0:
                  ...
                  if h_wall and f_wall:
                        print("first wall seen")
                        f_wall = False
                        seen_wall = True
                        move_tw == False
                        rotate_dir = -1
                  elif h_wall and not turn_corner:
                        if not -0.1 < model_grad < 0.1 and not trail_wall:
                              print(model_grad)
                              twist.angular.z = rotate_dir*0.1
                        elif not -0.1 < model_grad < 0.1 and trail_wall:
                              if model_grad >= 0.11:
                                    twist.angular.z = 1*0.1
                              elif model_grad <= -0.11:
                                    twist.angular.z = -1*0.1
                        elif move_tw == True:
                              twist.linear.x = 2.0;
                              if s_l_wall:
                              ...
                              if m_counter <= 6:
                                    ...
                                    walltoTrijectory = False
                        elif -0.1 <= model_grad <= 0.1:
                              move_tw == True
                  elif trail_wall and ((not s_l_wall) and (not s_r_wall)) and not turn_corner:
                        print("no side wall")
                        move_tw = False
                        turn_corner = True
                  elif turn_corner:
                        if steps <= 12:
                              ...
                              twist.linear.x = 2.0
                              c_yaw = yaw
                              if  round((c_yaw+corner_to_turn*2.1), 2) > 3.14:
                                     cal_yaw = ((c_yaw+corner_to_turn*2.1)%3.14)-3.14
                              else:
                                     cal_yaw = (c_yaw+corner_to_turn*2.1)
                        elif not round(yaw, 2) == round(cal_yaw, 2):
                              print(round(cal_yaw, 2), " -> ", round(yaw, 2))
                              twist.angular.z = -(yaw - cal_yaw)
                        elif (corner_to_turn == 1 and (not s_l_wall)) or (corner_to_turn == -1 and (not s_r_wall)):
                              print("wow")
                              twist.linear.x = 2.0
                        else:
                              print("turn corner done")
                              steps = 0
                              move_tw = True
                              turn_corner = False
                  elif move_tw == True:
